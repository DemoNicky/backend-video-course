...
import com.binar.backendonlinecourseapp.DTO.Response.GetUserResponse;
...
public class OrderController {
    ...
    @GetMapping(path = "/active-users")
    public ResponseEntity<ResponseHandling<List<GetUserResponse>>> getActiveUser() {
        ResponseHandling<List<GetUserResponse>> response = orderService.getActiveUser();
        if(response.getData() == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
        }
        return ResponseEntity.status(HttpStatus.OK).body(response);
